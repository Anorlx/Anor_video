<template>
  <div class="bg-surface rounded-lg border border-border shadow-sm overflow-hidden h-full">
    <!-- 视频信息头部 -->
    <div class="flex flex-col gap-5 p-4 sm:p-5">
      <div class="relative w-full aspect-video rounded-md overflow-hidden bg-surface-muted">
        <img
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="flex flex-wrap items-center gap-3 text-sm text-text-secondary mb-3">
          ...
          <span class="inline-flex items-center gap-1 px-2 py-0.5 bg-primary-light text-primary rounded-md text-xs font-medium">
            {{ video.platform }}
          </span>
    ...
    <!-- 格式选择 -->
    <div class="border-t border-border-light px-4 py-5 sm:px-5">
      ...
      <div class="grid grid-cols-1 gap-2">
        <button
          ...
          :class="[
            'flex items-center gap-3 px-3 py-3 rounded-md border text-left transition-all cursor-pointer',
            selectedFormat === fmt.format_id
              ? 'border-primary bg-primary-light ring-1 ring-primary/20'
              : 'border-border-light hover:border-primary/40 hover:bg-surface-muted'
          ]"
        >
          <div class="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center"
            :class="selectedFormat === fmt.format_id ? 'bg-primary text-white' : 'bg-surface-muted text-text-muted'">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
      ...
      <div class="mt-6 flex flex-col items-stretch gap-3">
        <button
          ...
          class="w-full inline-flex items-center justify-center gap-2 h-12 px-10 rounded-md bg-primary hover:bg-primary-dark text-white font-semibold text-base transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm cursor-pointer"
        >
        ...
        <button
          ...
          class="w-full inline-flex items-center justify-center gap-2 h-12 px-8 rounded-md border border-primary text-primary hover:bg-primary hover:text-white font-semibold text-base transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-transparent disabled:hover:text-primary"
        >
