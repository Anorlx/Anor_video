<template>
  <div class="bg-surface rounded-lg border border-border shadow-sm overflow-hidden h-full flex flex-col">
        <!-- 标签页导航 -->
        <div class="flex border-b border-border-light bg-surface-muted/50 overflow-x-auto">
          <button
            v-for="tab in tabs"
            :key="tab.key"
            @click="activeTab = tab.key"
            :class="[
              'flex items-center gap-2 px-4 sm:px-5 py-3.5 text-sm font-medium transition-all relative cursor-pointer whitespace-nowrap',
              activeTab === tab.key
                ? 'text-primary'
        ...
        <!-- 内容区域 -->
        <div class="p-4 sm:p-5 min-h-[400px] flex-1 overflow-y-auto">
          <!-- 加载状态 -->
            ...
            <div v-if="subtitleData.segments && subtitleData.segments.length > 0">
              <div class="flex items-center justify-between mb-4">
                <div class="text-sm text-text-secondary">
                  ...
                  <span v-if="subtitleData.language" class="ml-2 px-2 py-0.5 bg-primary-light text-primary rounded-md text-xs">
            ...
            <div v-if="mindmapMarkdown" class="relative">
              ...
              <div
                ref="mindmapContainer"
                  class="mindmap-wrapper w-full border border-border-light rounded-md bg-surface overflow-hidden"
            ...
            <div class="space-y-4">
              ...
              <div class="flex gap-2 pt-3 border-t border-border-light">
                <input
                  ...
                  type="text"
                  placeholder="输入你的问题..."
                  class="flex-1 h-11 px-4 rounded-md border border-border bg-surface text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all"
                  :disabled="chatLoading"
                />
                <button
                  @click="sendQuestion"
                  :disabled="!chatInput.trim() || chatLoading"
                  class="h-11 px-5 rounded-md bg-primary hover:bg-primary-dark text-white text-sm font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer flex items-center gap-1.5"
                >
...
const tabs = [
  ...
  { key: 'subtitle', label: '字幕文本', icon: 'M7 8h10M7 12h8M7 16h5M5 4h14a2 2 0 012 2v12a2 2 0 01-2 2H5a2 2 0 01-2-2V6a2 2 0 012-2z' },
  { key: 'mindmap', label: '思维导图', icon: 'M6 6h4v4H6V6zm8 0h4v4h-4V6zM6 14h4v4H6v-4zm8 0h4v4h-4v-4zm-4-6h4m-6 2v4m8-4v4m-6 2h4' },
  { key: 'qa', label: 'AI 问答', icon: 'M8 10h8M8 14h5m8-2a9 9 0 11-4.219-7.626L21 4l-1.374 4.219A8.963 8.963 0 0121 12z' },
]
